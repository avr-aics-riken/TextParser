...
int read_write_parameters(std::string ifilename, std::string ofilename)
{
  int stat = 0;
  ...
  if(tp!=0){
    ...
    if (stat == 0) {
      ...
      if (stat == 0) {
        std::cout << "# Successful completion of writing a file : " << ofilename << std::endl;
      } else {
        std::cout << "# Error occurs during writing a file : C" << stat << std::endl;
        delete tp;
        return 1;
      }
    } else {
