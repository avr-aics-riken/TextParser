...
! status = status + move_and_get_parameters('Input0-1.txt',tp_ptr)
  status = status + move_and_get_parameters('./tpp_examples/correct_basic_1.txt',tp_ptr)
  status = status + tp_delete_instance(tp_ptr)

  status = status + tp_get_instance_singleton(tp_ptr)
! status = status + move_and_get_parameters('Input4-1.txt',tp_ptr)
  status = status + move_and_get_parameters('./tpp_examples/correct_basic_12.txt',tp_ptr)
...
recursive function move_and_get_parameters(filename,tp_ptr) RESULT(status)
  ...
  st = TP_READ(tp_ptr, filename)
  status = status + st

  if (st /= 0) then
    write(6,*) 'TP_READ status                 : ', st
  end if
...
recursive subroutine get_directory_parameters(filename, l_label, label, tp_ptr)
  ...
  write(6,*) 'Num. of (daughter) Nodes       : ', dir_number
